...
const OrderSection = () => {
  ...
  const handleOrderClick = (order) => {
    if (order.status === 'PAID' || order.status === 'PENDING') {
      navigate(`/success?external_id=${order.id}`);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 md:py-32">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Syncing Transactions...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700 px-1 md:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 md:mb-4">Total Investment</p>
          <h3 className="text-2xl md:text-3xl font-black text-slate-900 italic tracking-tighter">
            Rp {totalSpent.toLocaleString('id-ID')}
          </h3>
          <div className="mt-4 flex items-center gap-2">
            <span className="text-[9px] font-black bg-emerald-500/10 text-emerald-600 px-2 py-1 rounded-lg uppercase tracking-tighter">Success Payment</span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 md:mb-4">Waiting Action</p>
          <h3 className="text-2xl md:text-3xl font-black text-slate-900 italic tracking-tighter">
            {pendingOrdersCount} <span className="text-sm font-bold text-slate-400 uppercase tracking-normal">Orders</span>
          </h3>
          <div className="mt-4 flex items-center gap-2">
            <span className="text-[9px] font-black bg-amber-500/10 text-amber-600 px-2 py-1 rounded-lg uppercase tracking-tighter">Pending Approval</span>
          </div>
        </div>
        ...
        <div className="bg-slate-900 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-xl shadow-purple-200 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 md:mb-4">Total Transactions</p>
          ...
          <div className="mt-4 flex items-center gap-2">
            <span className="text-[9px] font-black bg-white/10 text-purple-400 px-2 py-1 rounded-lg uppercase tracking-tighter italic font-black">History Log</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/60 overflow-hidden">
        <div className="p-6 md:p-8 lg:p-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="text-lg md:text-xl font-black text-slate-900 uppercase italic tracking-tighter">Order History</h4>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Manage bookings</p>
            </div>
          </div>
        ...
        <div className="divide-y divide-slate-50">
          {orders.length === 0 ? (
            <div className="py-20 md:py-24 flex flex-col items-center">
              ...
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em]">Empty Vault</p>
            </div>
          ) : (
            orders.map((order) => (
              <div 
              ...
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 md:mt-3 flex items-center gap-4">
                        <div className="flex -space-x-2">
                            {[...Array(Math.min(Number(order.quantity), 3))].map((_, i) => (
                        ...
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{order.quantity} Tickets</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between w-full lg:w-auto lg:gap-12 border-t lg:border-t-0 pt-5 lg:pt-0 border-slate-100">
                  <div className="text-left lg:text-right">
                    <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Transaction</p>
