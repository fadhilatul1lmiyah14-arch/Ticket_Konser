...
import { 
  ...
  ShieldCheck, LayoutGrid, Layers, RefreshCcw, MapPin, Languages, Sparkles, Info,
  Crown, Star, ArrowRight, FileText
} from 'lucide-react';
...
const EditConcert = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTermsList(Array.isArray(termsData) ? termsData : (termsData ? [termsData] : []));
  ...
  const handleDescriptionChange = (index, content) => {
    setDescriptions(prev => {
      const newDescs = [...prev];
      newDescs[index] = { ...newDescs[index], [activeLang]: content };
      return newDescs;
    });
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="font-black text-slate-400 uppercase tracking-widest text-xs animate-pulse">Synchronizing Data...</p>
      </div>
    );
  }

  return (
    <form onSubmit={handleUpdate} className="max-w-7xl mx-auto pb-20 px-4 sm:px-6 lg:px-8 bg-white text-left animate-fade-in-up">
      ...
      <header className="mb-8 md:mb-12 flex flex-col md:flex-row justify-between items-start md:items-center border-b border-purple-100 pb-8 md:pb-10 gap-6 md:gap-8">
        <div className="space-y-1 w-full md:w-auto">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-50 to-pink-50 px-4 py-2 rounded-full border border-purple-100 mb-3">
            <RefreshCcw size={12} className="text-purple-500 animate-spin-slow" />
            <span className="w-2 h-2 bg-purple-500 rounded-full animate-ping"></span>
            <h2 className="text-[8px] md:text-[9px] font-black text-purple-600 uppercase tracking-[0.3em]">Update Mode</h2>
          </div>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black uppercase italic tracking-tighter leading-none">
            Edit <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600 bg-[length:200%_auto] animate-gradient-x">Concert</span>
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 md:gap-4 w-full md:w-auto">
          ...
          <button type="button" onClick={() => navigate(-1)} className="p-3 md:p-5 bg-white border border-purple-100 text-slate-400 rounded-[18px] md:rounded-[24px] hover:text-rose-500 hover:border-rose-200 transition-all shadow-sm group hover:rotate-90">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
        <div className="lg:col-span-8 space-y-8 md:space-y-10">
          ...
          <div className="bg-white p-6 sm:p-8 md:p-12 rounded-[32px] md:rounded-[48px] shadow-2xl shadow-purple-100/40 border border-purple-100 relative overflow-hidden group">
            ...
            <div className="absolute top-6 right-6 flex items-center gap-2 text-[10px] font-bold text-purple-600 bg-purple-100 px-3 py-1 rounded-full">
            ...
            <h3 className="font-black text-slate-800 uppercase italic mb-6 md:mb-10 flex items-center gap-3 md:gap-4 text-lg md:text-xl">
              <LayoutGrid size={22} className="text-purple-500" /> Core Configuration
            </h3>
            
            <div className="space-y-6 md:space-y-8 relative z-10">
              <div className="group">
                <label className="text-[10px] md:text-[11px] font-black uppercase text-slate-500 mb-2 md:mb-3 block ml-4 group-focus-within:text-purple-500 transition-colors">Concert Title ({activeLang.toUpperCase()})</label>
                <input 
              ...
              <div className="group">
                <div className="flex justify-between items-center mb-2 md:mb-3 ml-4">
                  ...
                  <button type="button" onClick={addDescriptionBlock} className="text-[9px] font-black bg-purple-100 text-purple-600 px-3 py-1 rounded-full hover:bg-gradient-to-r hover:from-purple-600 hover:to-pink-600 hover:text-white transition-all flex items-center gap-1">
                  ...
                  {descriptions.map((desc, idx) => (
                    <div key={idx} className="relative group/editor animate-in slide-in-from-top-2">
                      ...
                      {descriptions.length > 1 && (
                        <button type="button" onClick={() => removeDescriptionBlock(idx)} className="absolute -right-2 -top-2 p-2 bg-rose-500 text-white rounded-full shadow-lg opacity-0 group-hover/editor:opacity-100 transition-opacity z-20 hover:scale-110">
                          <X size={14}/>
                        </button>
                      )}
                    </div>
              ...
              <div className="pt-4 md:pt-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                  ...
                  <button type="button" onClick={addTicketTier} className="flex items-center gap-2 text-[9px] md:text-[10px] font-black bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-full hover:shadow-lg transition-all shadow-md">
                  ...
                  {ticketTiers.map((tier, index) => (
                    <div key={index} className="flex flex-col md:flex-row gap-4 bg-gradient-to-r from-purple-50/30 to-pink-50/30 p-4 md:p-6 rounded-[24px] md:rounded-[28px] border border-purple-100 group hover:bg-white hover:shadow-xl transition-all animate-in slide-in-from-left-2">
                      <div className="flex-1">
                        ...
                        <input type="text" placeholder="e.g. VIP" required value={tier.name} onChange={(e) => handleTierChange(index, 'name', e.target.value)} className="w-full bg-white border border-purple-200 rounded-xl md:rounded-2xl px-4 py-2 md:py-3 font-bold text-slate-700 outline-none focus:border-purple-400 text-sm"/>
                      </div>
                      <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
                        <div className="md:w-40">
                          <label className="text-[8px] md:text-[9px] font-bold text-slate-500 uppercase mb-1 block ml-2">Price (IDR)</label>
                          <input type="number" required value={tier.price} onChange={(e) => handleTierChange(index, 'price', e.target.value)} className="w-full bg-white border border-purple-200 rounded-xl md:rounded-2xl px-4 py-2 md:py-3 font-bold text-slate-700 outline-none focus:border-purple-400 text-sm"/>
                        </div>
                        <div className="md:w-28">
                          <label className="text-[8px] md:text-[9px] font-bold text-slate-500 uppercase mb-1 block ml-2">Seats</label>
                          <input type="number" required value={tier.quota} onChange={(e) => handleTierChange(index, 'quota', e.target.value)} className="w-full bg-white border border-purple-200 rounded-xl md:rounded-2xl px-4 py-2 md:py-3 font-bold text-slate-700 outline-none focus:border-purple-400 text-sm"/>
                        </div>
                      </div>
                      <div className="flex items-end justify-end md:justify-start pb-1">
                        <button type="button" onClick={() => removeTicketTier(index)} className={`p-3 rounded-xl transition-all ${ticketTiers.length > 1 ? 'text-rose-400 hover:bg-rose-50 hover:text-rose-600' : 'text-slate-200 cursor-not-allowed'}`}>
                          <Trash2 size={20}/>
                        </button>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 pt-4 md:pt-6">
                <div className="space-y-3">
                  ...
                  <div className="relative group">
                    ...
                    <input type="date" name="event_date" required value={formData.event_date} onChange={handleChange} className="w-full bg-gradient-to-r from-slate-50 to-white border-2 border-purple-100 focus:border-purple-400 rounded-[20px] md:rounded-[24px] py-4 md:py-6 pl-14 md:pl-16 pr-6 md:pr-8 font-black outline-none text-slate-700 shadow-inner transition-all uppercase text-xs md:text-sm" />
                  </div>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="relative group">
                    ...
                    <input type="time" name="start_time" required value={formData.start_time} onChange={handleChange} className="w-full bg-gradient-to-r from-slate-50 to-white border-2 border-purple-100 focus:border-purple-400 rounded-[20px] md:rounded-[24px] py-4 md:py-6 pl-14 md:pl-16 pr-6 md:pr-8 font-black outline-none text-slate-700 shadow-inner transition-all text-xs md:text-sm" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="relative z-10">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                ...
                <div className="flex items-center gap-2 bg-purple-500/20 border border-purple-500/30 px-4 py-2 rounded-full">
                  ...
                  <span className="text-[10px] font-bold text-purple-200 uppercase tracking-tight">S&K Tambahan Khusus Event Ini</span>
                </div>
              </div>
              ...
              <div className="flex gap-2 md:gap-3 mb-8">
                ...
                <button type="button" onClick={addTerm} className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 md:px-8 rounded-[18px] md:rounded-[22px] hover:scale-105 active:scale-95 transition-all shadow-lg flex items-center justify-center">
                ...
                {termsList.length > 0 ? (
                  termsList.map((t, i) => (
                    <div key={i} className="flex items-center justify-between bg-white/5 hover:bg-white/10 p-4 rounded-xl md:rounded-2xl border border-white/5 transition-colors group animate-in zoom-in duration-300">
                      ...
                      <button type="button" onClick={() => setTermsList(termsList.filter((_, idx) => idx !== i))} className="p-1 hover:bg-rose-500/20 rounded-md transition-colors text-white/20 hover:text-rose-400">
                        <X size={16} />
                      </button>
                    </div>
                  ))
                ) : (
                  <div className="col-span-full py-10 border-2 border-dashed border-white/5 rounded-[32px] flex flex-col items-center justify-center opacity-40">
        ...
        <div className="lg:col-span-4 space-y-8 md:space-y-10">
          ...
          <div className="bg-white p-6 md:p-8 rounded-[32px] md:rounded-[40px] shadow-2xl shadow-purple-100/40 border border-purple-100">
            ...
            <div className="space-y-5 md:space-y-6">
              <div>
                ...
                <select name="category_id" required value={formData.category_id} onChange={handleChange} className="w-full bg-gradient-to-r from-slate-50 to-white border-2 border-purple-100 focus:border-purple-400 rounded-[18px] md:rounded-[22px] py-4 md:py-5 px-6 md:px-8 font-black uppercase text-[11px] md:text-[12px] outline-none text-slate-700 transition-all appearance-none">
              ...
              <div>
                ...
                <select name="location_id" required value={formData.location_id} onChange={handleChange} className="w-full bg-gradient-to-r from-slate-50 to-white border-2 border-purple-100 focus:border-purple-400 rounded-[18px] md:rounded-[22px] py-4 md:py-5 px-6 md:px-8 font-black uppercase text-[11px] md:text-[12px] outline-none text-slate-700 transition-all appearance-none mb-4">
                ...
                <div className="relative group mb-4">
                  <MapPin className="absolute left-5 top-5 z-10 text-purple-500" size={16} />
                  <textarea 
                    ...
                    className="w-full bg-gradient-to-r from-slate-50 to-white border-2 border-purple-100 focus:border-purple-400 rounded-[18px] md:rounded-[22px] py-4 pl-12 pr-6 font-bold text-[11px] outline-none text-slate-700 transition-all resize-none"
                  />
                </div>
                ...
                <div className="space-y-3 mt-4">
                  <div className="flex justify-between items-center px-1">
                    <label className="text-[9px] font-black text-purple-600 uppercase tracking-widest flex items-center gap-2">
                      <MapPin size={12}/> Map Geolocation
                    </label>
                    <div className="flex gap-2 text-[8px] font-black text-purple-500 bg-purple-50 px-2 py-1 rounded-md">
                      <span>LAT: {formData.latitude.toFixed(4)}</span>
                      <span>LNG: {formData.longitude.toFixed(4)}</span>
                    </div>
                  </div>
                  <div className="h-[220px] w-full rounded-[20px] overflow-hidden border-2 border-purple-100 shadow-sm relative z-0">
                  ...
                  <p className="text-[8px] text-slate-400 italic px-2">*Click map to update location</p>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-purple-100">
                ...
                <div className="flex gap-2 mb-4 md:mb-6">
                  <input type="text" value={inputCast} onChange={(e) => setInputCast(e.target.value)} placeholder="Artist Name" className="flex-1 bg-gradient-to-r from-slate-50 to-white border-2 border-purple-100 focus:border-purple-400 rounded-[14px] md:rounded-[18px] px-4 md:px-6 py-3 md:py-4 text-xs font-bold outline-none text-slate-700" />
          ...
          <div className="bg-white p-6 md:p-8 rounded-[32px] md:rounded-[40px] shadow-2xl shadow-purple-100/40 border border-purple-100 relative">
            ...
            <div className="space-y-5 md:space-y-6">
              <div className="flex gap-2">
                <input type="url" value={inputUrl} onChange={(e) => setInputUrl(e.target.value)} placeholder="URL Image" className="flex-1 bg-gradient-to-r from-slate-50 to-white border-2 border-purple-100 focus:border-purple-400 rounded-[14px] md:rounded-[18px] px-4 md:px-6 py-3 md:py-4 text-xs font-bold outline-none text-slate-700" />
          ...
          >
            ...
            {isUpdating ? <Loader2 className="animate-spin relative z-10" size={24} /> : <><ShieldCheck size={20} className="relative z-10 group-hover:rotate-12 transition-transform"/> <span className="relative z-10">Update Record</span> <ArrowRight size={18} className="relative z-10 group-hover:translate-x-1 transition-transform"/></>}
          </button>
        </div>
      </div>
